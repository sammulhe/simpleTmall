	...
	<div class="listDataTableDiv">
		...
		<div style="text-align:right">
            ...
            <c:if test="${page.start-page.count>=0}">
               <a href="?start=${page.start-page.count }&cid=${category.id }">上一页</a>
            </c:if>
            <c:if test="${page.start-page.count<0}">  <!--防止越界  -->
               <a href="?start=0&cid=${category.id }">上一页</a>        
            </c:if>
            <c:if test="${page.start+page.count<=page.last}">
              <a href="?start=${page.start+page.count }&cid=${category.id }">下一页</a>
            </c:if>
            <c:if test="${page.start+page.count>page.last}">
              <a href="?start=${page.last }&cid=${category.id }">下一页</a> <!-- 防止越界 -->
            </c:if>
            <a href="?start=${page.last }&cid=${category.id }">末页</a>
       </div>
	</div>
<%-- 
	<div class="pageDiv">
